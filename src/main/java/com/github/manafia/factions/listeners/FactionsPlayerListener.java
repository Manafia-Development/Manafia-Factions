...
import com.github.manafia.factions.util.FlightEnhance;
import com.github.manafia.factions.util.VisualizeUtil;
import com.github.manafia.factions.util.wait.WaitExecutor;
...
public class FactionsPlayerListener implements Listener {
    ...
    public static boolean canPlayerUseBlock(Player player, Block block, boolean justCheck) {
        ...
        if (!otherFaction.getId().equals(myFaction.getId())) { // If the faction
            // target is not
            // my own
    ...
    public static boolean preventCommand(String fullCmd, Player player) {
        if ((Conf.territoryNeutralDenyCommands.isEmpty() && Conf.territoryEnemyDenyCommands.isEmpty() && Conf.permanentFactionMemberDenyCommands
                .isEmpty() && Conf.warzoneDenyCommands.isEmpty()))
            return false;
        ...
        if (at.isWilderness() && !Conf.wildernessDenyCommands.isEmpty() && !me.isAdminBypassing() && isCommandInList(fullCmd, shortCmd,
                Conf.wildernessDenyCommands.iterator())) {
            me.msg(TL.PLAYER_COMMAND_WILDERNESS, fullCmd);
            return true;
        }

        Relation rel = at.getRelationTo(me);
        if (at.isNormal() && rel.isAlly() && !Conf.territoryAllyDenyCommands.isEmpty() && !me.isAdminBypassing() && isCommandInList(fullCmd, shortCmd,
                Conf.territoryAllyDenyCommands.iterator())) {
            me.msg(TL.PLAYER_COMMAND_ALLY, fullCmd);
            return true;
        }

        if (at.isNormal() && rel.isNeutral() && !Conf.territoryNeutralDenyCommands.isEmpty() && !me.isAdminBypassing() && isCommandInList(fullCmd,
                shortCmd, Conf.territoryNeutralDenyCommands.iterator())) {
            me.msg(TL.PLAYER_COMMAND_NEUTRAL, fullCmd);
            return true;
        }

        if (at.isNormal() && rel.isEnemy() && !Conf.territoryEnemyDenyCommands.isEmpty() && !me.isAdminBypassing() && isCommandInList(fullCmd,
                shortCmd, Conf.territoryEnemyDenyCommands.iterator())) {
            me.msg(TL.PLAYER_COMMAND_ENEMY, fullCmd);
            return true;
        }

        if (at.isWarZone() && !Conf.warzoneDenyCommands.isEmpty() && !me.isAdminBypassing() && isCommandInList(fullCmd, shortCmd,
                Conf.warzoneDenyCommands.iterator())) {
    ...
    public void onPowerRegen(PowerRegenEvent e) {
        ...
        Booster booster = e.getFaction().getActiveBoosters().get(BoosterType.POWER);
        double addedPower = e.getDelta() * booster.getMultiplier();
        e.setDelta(addedPower);
    }
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        // clear visualization
        if (event.getFrom().getBlockX() != event.getTo().getBlockX() || event.getFrom().getBlockY() != event.getTo().getBlockY() || event.getFrom()
                .getBlockZ() != event.getTo().getBlockZ()) {
        ...
        if (me.getAutoClaimFor() != null) {
            if (FactionsPlugin.cachedRadiusClaim && me.attemptClaim(me.getFaction(), me.getPlayer().getLocation(), true))
                me.getFaction().getFPlayersWhereOnline(true).forEach(f -> f.msg(TL.CLAIM_CLAIMED, me.describeTo(f, true), me.getFaction().describeTo(
                        f), at.describeTo(f)));
            else
                me.attemptClaim(me.getAutoClaimFor(), me.getPlayer().getLocation(), true);
            Util.logFactionEvent(me.getAutoClaimFor(), FLogType.CHUNK_CLAIMS, me.getName(), CC.GreenB + "CLAIMED", String.valueOf(1), (new FLocation(
                    player.getLocation())).formatXAndZ(","));
        ...
        if (changedFaction) {
            Bukkit.getScheduler().runTask(FactionsPlugin.getInstance(), () -> Bukkit.getServer().getPluginManager().callEvent(
                    new FPlayerEnteredFactionEvent(factionTo, factionFrom, me)));
            if (FactionsPlugin.getInstance().getConfig().getBoolean("Title.Show-Title")) {
                ...
                String subTitle = FactionsPlugin.getInstance().getConfig().getString("Title.Format.Subtitle").replace("{Description}", factionTo
                        .getDescription()).replace("{Faction}", factionTo.getColorTo(me) + factionTo.getTag());
                ...
                if (FactionsPlugin.getInstance().version != 7) {
                    Bukkit.getScheduler().runTaskLater(FactionsPlugin.getInstance(), () -> {
                        if (FactionsPlugin.getInstance().version != 8) {
                            me.getPlayer().sendTitle(Util.color(finalTitle), Util.color(finalsubTitle), FactionsPlugin.getInstance().getConfig()
                                            .getInt("Title.Options.FadeInTime"), FactionsPlugin.getInstance().getConfig().getInt("Title.Options.ShowTime"),
        ...
        if (me.isMapAutoUpdating()) {
            if (!showTimes.containsKey(player.getUniqueId()) || (showTimes.get(player.getUniqueId()) < System.currentTimeMillis())) {
                ...
                showTimes.put(player.getUniqueId(), System.currentTimeMillis() + FactionsPlugin.getInstance().getConfig().getInt(
                        "findfactionsexploit.cooldown"));
            }
        } else {
            Faction myFaction = me.getFaction();
    ...
    public void onInspect(PlayerInteractEvent e) {
        if (e.getAction().name().contains("BLOCK")) {
            ...
            player.sendMessage(TL.COMMAND_INSPECT_HEADER.toString().replace("{x}", e.getClickedBlock().getX() + "").replace("{y}", e.getClickedBlock()
                    .getY() + "").replace("{z}", e.getClickedBlock().getZ() + ""));
            ...
            for (String[] strings : info) {
                ...
                player.sendMessage(rowFormat.replace("{time}", convertTime(row.getTime())).replace("{action}", row.getActionString()).replace(
                        "{player}", row.getPlayer()).replace("{block-type}", row.getType().toString().toLowerCase()));
            }
        }
    }
    ...
    public void onClose(InventoryCloseEvent e) {
        ...
        if (fme.isInFactionsChest())
            fme.setInFactionsChest(false);
    }
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        // Creeper Egg Bypass.
        if (Conf.allowCreeperEggingChests && block.getType() == Material.CHEST && type == XMaterial.CREEPER_SPAWN_EGG.parseMaterial() && event
                .getPlayer().isSneaking())
            return;

        // territoryBypasssProtectedMaterials totally bypass the protection
        // system
        if (Conf.territoryBypassProtectedMaterials.contains(block.getType()))
            return;
        // Do type null checks so if XMaterial has a parsing issue and fills
        // null as a value it will not bypass.
        // territoryCancelAndAllowItemUseMaterial bypass the protection system
        // but only if they're not clicking on territoryDenySwitchMaterials
        // if they're clicking on territoryDenySwitchMaterials, let the
        // protection system handle the permissions
        if (type != null && !Conf.territoryDenySwitchMaterials.contains(block.getType()))
            if (Conf.territoryCancelAndAllowItemUseMaterial.contains(event.getPlayer().getItemInHand().getType())
    ...
    public void onPlayerBoneMeal(PlayerInteractEvent event) {
        Block block = event.getClickedBlock();
        if (event.getAction() == Action.RIGHT_CLICK_BLOCK && block.getType() == XMaterial.GRASS_BLOCK.parseMaterial() && event.hasItem() && event
                .getItem().getType() == XMaterial.BONE_MEAL.parseMaterial())
            if (!FactionsBlockListener.playerCanBuildDestroyBlock(event.getPlayer(), block.getLocation(), "build", true)) {
                ...
                me.msg(TL.ACTIONS_NOPERMISSION.toString().replace("{faction}", myFaction.getTag(me.getFaction())).replace("{action}",
                        "use bone meal"));
                event.setCancelled(true);
            }
    }
    ...
    public void onPlayerRespawn(PlayerRespawnEvent event) {
        ...
        if (Conf.homesEnabled && Conf.homesTeleportToOnDeath && home != null && (Conf.homesRespawnFromNoPowerLossWorlds || !Conf.worldsNoPowerLoss
    ...
    public void AsyncPlayerChatEvent(AsyncPlayerChatEvent e) {
        ...
        if (CmdFGlobal.toggled.contains(p.getUniqueId()))
            ...
            if (FPlayers.getInstance().getByPlayer(p).getFaction() == null) {
                ...
                // are in a faction that's not Wilderness, SafeZone, or Warzone,
                // check their chat status
                if (!FPlayers.getInstance().getByPlayer(p).getChatMode().isAtLeast(ChatMode.ALLIANCE)) {
                    // their Faction Chat Mode is not at-least a Alliance, cancel
                    // and return
                    e.setCancelled(true);
                    return;
                }

        // we made it this far, since we didn't return yet, we must have sent
        // the chat event through
        // iterate through all of recipients and check if they're muted, then
        // remove them from the event list

        List<Player> l = new ArrayList<>(e.getRecipients());

        for (int i = l.size() - 1; i >= 0; i--) { // going backwards in the list
            // to prevent a
