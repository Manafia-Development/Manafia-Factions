...
import com.github.manafia.factions.event.PowerLossEvent;
...
public class FactionsEntityListener implements Listener {
     ...
     * enemies. I can hurt neutrals as long as
    ...
    public void onEntityDeath(EntityDeathEvent event) {
        ...
        if (!(entity instanceof Player)) {
            ...
            if (fplayer.getFaction().hasBooster(BoosterType.MOBDROPS)) {
                Booster booster = fplayer.getFaction().getActiveBoosters().get(BoosterType.MOBDROPS);
                spawnMoreDrops(event, booster.getMultiplier());
            } else if (fplayer.getFaction().hasBooster(BoosterType.EXP)) {
                Booster booster = fplayer.getFaction().getActiveBoosters().get(BoosterType.EXP);
                spawnMoreExp(event, booster.getMultiplier());
            }
            return;
        }
        ...
        // Call player onDeath if the event is not cancelled and not using
        // custom power
        ...
        if (event instanceof EntityDamageByEntityEvent) {
            ...
            if (damageee instanceof Player) {
                if (damager instanceof Player) {
                    ...
                    if ((fdamagee.getRelationTo(fdamager) == Relation.ALLY) || (fdamagee.getRelationTo(fdamager) == Relation.TRUCE) || (fdamagee
                            .getFaction() == fdamager.getFaction()))
                        return;
                } else

                    // this triggers if damageee is a player and damager is mob ( so
                    // like if a skeleton hits u )
                    if (damager instanceof Projectile)
                        ...
                        if (((Projectile) damager).getShooter() instanceof Player) {
                            ...
                            if (relation == Relation.ALLY || relation == Relation.TRUCE || fdamager.getFaction() == fdamagee.getFaction())
            ...
            if (damageee != null && damageee instanceof Player && (playerHurt && FactionsPlugin.instance.getConfig().getBoolean(
                    "ffly.disable-flight-on-player-damage", true) || (!playerHurt && FactionsPlugin.instance.getConfig().getBoolean(
                    "ffly.disable-flight-on-mob-damage", true)))) {
                cancelFStuckTeleport((Player) damageee);
                if (!combatList.contains(damageee.getUniqueId()))
                    combatList.add(damagee.getUniqueId());
                Bukkit.getScheduler().runTaskLater(FactionsPlugin.instance, () -> combatList.remove(damageee.getUniqueId()), 20
                        * FactionsPlugin.instance.getConfig().getInt("ffly.CombatFlyCooldown"));
            ...
            if (damager instanceof Player) {
                ...
                Bukkit.getScheduler().runTaskLater(FactionsPlugin.instance, () -> combatList.remove(finalDamager.getUniqueId()), 20
                        * FactionsPlugin.instance.getConfig().getInt("ffly.CombatFlyCooldown"));
    ...
    public boolean canDamagerHurtDamagee(EntityDamageByEntityEvent sub, boolean notify) {
        ...
        // Players can not take attack damage in a SafeZone or peaceful
        // territory depending on Conf options.
    ...
    public void onCreatureSpawn(CreatureSpawnEvent event) {
        ...
        if (Conf.safeZoneNerfedCreatureTypes.contains(event.getEntityType()) && Board.getInstance().getFactionAt(new FLocation(event.getLocation()))
    ...
    public void onPaintingBreak(HangingBreakEvent event) {
        if (event.getCause() == RemoveCause.EXPLOSION) {
            ...
            if ((faction.isWilderness() && !Conf.worldsNoWildernessProtection.contains(loc.getWorld().getName()) && (Conf.wildernessBlockCreepers
                    || Conf.wildernessBlockFireballs || Conf.wildernessBlockTNT)) || (faction.isNormal() && (online ? (Conf.territoryBlockCreepers
                    || Conf.territoryBlockFireballs || Conf.territoryBlockTNT)
                    : (Conf.territoryBlockCreepersWhenOffline || Conf.territoryBlockFireballsWhenOffline
                    || Conf.territoryBlockTNTWhenOffline))) || (faction.isWarZone() && (Conf.warZoneBlockCreepers
                    || Conf.warZoneBlockFireballs || Conf.warZoneBlockTNT)) || faction.isSafeZone()) {
    ...
    public void onEntityChangeBlock(EntityChangeBlockEvent e) {
        ...
        if (entity.getType() == EntityType.ENDERMAN)
            if (stopEndermanBlockManipulation(loc))
                ...
                Faction faction = Board.getInstance().getFactionAt(new FLocation(loc));
                // it's a bit crude just using fireball protection, but I'd
                // rather not add in a whole new set of xxxBlockWitherExplosion
                // or whatever
    ...
    // For disabling interactions with item frames in another faction's
    // territory
    @EventHandler(priority = EventPriority.NORMAL, ignoreCancelled = true)
